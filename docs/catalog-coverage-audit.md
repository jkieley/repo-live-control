...
The read-only runtime audit proves resource availability and component identity. It is not a live 2.2.0 spawning or two-client acceptance test. After loading 2.2.0 in a gameplay host session, check one case, one soul, and one money bag with count `1` and `player-location`, confirm their actual in-world behavior, then despawn each tracked target. Test peer visibility separately before claiming multiplayer acceptance for these additions.
